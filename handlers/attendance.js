...
function registerAttendanceHandlers(bot) {
  ...
  bot.action("confirm_attendance", async (ctx) => {
    try {
      ...
      if (
        !ctx.session.selectedStudents ||
        ctx.session.selectedStudents.length === 0
      ) {
...
Tasdiqlaysizmi?
  ...
  bot.action("save_attendance", async (ctx) => {
    try {
      ...
      const resultMessage = `
📝 <b>Davomat Tasdiqlash</b><br><br>
🏫 Sinf: ${className}<br>
⏰ Soat: ${hour}<br>
📅 Sana: ${formatDate(getCurrentDate())}<br><br>
👥 <b>Tanlangan o'quvchilar (${selectedStudents.length} ta):</b><br>
${studentNames
